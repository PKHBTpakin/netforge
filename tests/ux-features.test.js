...
/* ---------- ตัวควบคุมที่ระบบปฏิบัติการวาดเอง ----------
   ที่มา: ผู้ใช้เปิดรายการเลือกความเร็วสัญญาณนาฬิกาแล้วอ่านตัวเลขไม่ออก
   วัดจากภาพหน้าจอจริงได้พื้น rgb(191,191,191) กับตัวอักษร rgb(216,218,224) ต่างกันแค่ 1.32 ต่อ 1

   สาเหตุ: รายการที่กางออกมาของ select ถูกวาดโดยระบบปฏิบัติการ ไม่ใช่โดยหน้าเว็บ
   พื้นจึงเป็นสีเทาอ่อนของค่าเริ่มต้น ขณะที่ตัวอักษรสืบทอดสีอ่อนมาจาก .input-cyber
   เห็นอ่านออกเฉพาะบรรทัดที่ถูกไฮไลต์ เพราะเบราว์เซอร์ทับสีให้เองเฉพาะบรรทัดนั้น

   เรื่องนี้ CSS ปกติเข้าไปแก้ไม่ได้ ต้องประกาศ color-scheme บอกเบราว์เซอร์ตรง ๆ
   ซึ่งแก้ปุ่มลูกศรของช่องตัวเลขและแถบเลื่อนภายในตัวควบคุมไปพร้อมกันด้วย */

check('ตัวควบคุมระบบ: ประกาศ color-scheme ครบทั้งสองธีม',
...
/* สีที่ใช้กับรายการต้องอ่านออกจริงทั้งสองธีม ไม่ใช่แค่ประกาศไว้เฉย ๆ */
[['มืด', darkBlock], ['สว่าง', lightBlock]].forEach(function (pair) {
    const c = contrast(cssVar(pair[1], 'text'), cssVar(pair[1], 'card'));
    check('ตัวควบคุมระบบ: รายการของ select อ่านออกในโหมด' + pair[0],
        c >= 4.5, c.toFixed(2));
});
